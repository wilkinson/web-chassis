 ...
 // Module definition
    ...
    q.base = function () {
     ...
     // Declarations
        ...
        var duck, fakeObject, generic, guts, key_gen, known_types, typeIndex;
     ...
     // Definitions
        ...
        if (typeof Array.prototype.indexOf === 'function') {
            ...
            typeIndex = function (x) {
                ...
                return {
                    special:    (i !== -1),
                    index:      (i !== -1) ? i : typeIndex(x.constructor).index
                };
            };

            q.base$registerType = function (x) {
                if (Array.prototype.indexOf.call(known_types, x) !== (-1)) {
                    known_types.push(x);
                }
            };
     ...
     // NOTE: q.base$registerType has already been implemented above.
        ...
        q.base$format(Number).def = function (x) {
         // See also: x.toExponential, x.toFixed, x.toString ...
            if (q.flags.hasOwnProperty("digits")) {
                return x.toPrecision(parseInt(q.flags.digits));
            } else {
     ...
     // And while we're at it, we'll go ahead and replace one of the builtins.
        ...
        (function (puts) {
            q.puts = function () {
                var temp = q.base$map(arguments).using(q.base$format);
